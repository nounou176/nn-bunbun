...
- Add a closed repair context. Attempt 1 requires `repair: null`. Attempt 2
  ...
  failures explicitly use `priorResult: null`; no raw malformed response is
  embedded in a repair request.
...
packet. It includes the prior structured result only when strict parsing
succeeded, the prior raw-response hash, the failure stage, and bounded stable
diagnostics generated by local code. An invalid attempt 2 ends the compilation.
This makes malformed JSON repairable without treating a partially parsed
object as accepted truth.
...
### 6. Verify, manually accept, and close Milestone 7

Run all supported static/unit/integration/build checks, hand the user the
manual browser matrix, record only reported observations, update durable docs
and shared memory, mark M7 complete in CURRENT_STATE/ROADMAP, and leave M8 as
the next milestone.

Observable checkpoint: all five M7 exit criteria have direct code/test/manual
evidence and no compiler requirement remains hidden in chat history.
...
Run from `/home/nunu/Desktop/nnlab/nn-bunbun` with Node 24.18.0 active:

    export PATH=/home/nunu/.nvm/versions/node/v24.18.0/bin:$PATH
...
Expected outcomes are zero schema drift, zero plugin/prompt/media-policy drift,
all contract/compiler/persistence/web tests passing, no TypeScript or ESLint
errors, and successful server/web production builds. Vite's already recorded
large-chunk warning may remain visible and must be reported rather than hidden.

No Playwright command applies because D-011 requires user-run browser and
gameplay validation. No Docker build applies because the repository has no
Dockerfiles and D-015 keeps release work after local acceptance.
...
| ----------------------------------------------------------- | ------- | ------- | ------- | ----------------------- |
| Full export → Skill → file import → review → publish → play | Pending | Pending | Not run | Awaiting implementation |
| Invalid import and one bounded repair                       | Pending | Pending | Not run | Awaiting implementation |
| Offline published lesson and authored-demo regression       | Pending | Pending | Not run | Awaiting implementation |
| Renderer/audio/persistence/reload edge matrix               | Pending | Pending | Not run | Awaiting implementation |
...
configured local marketplace, and testing occurs in a new thread.
...
- Add accepted D-034 and update deferred O-006/O-009 wording in
  `docs/DECISIONS.md` after user approval.
- Version `docs/ai-modules/CONTRACT.md`, the plugin protocol/runbook, evaluation
  report, `docs/AI_MODULES.md`, and `docs/M7_VARIANTS.md` to reflect 0.2.0 and
  actual requalification evidence.
- Update `docs/BUNBUN_ARCHITECTURE.md`, `docs/LESSON_MANIFEST.md`, and relevant
  gameplay/privacy/compiler sections with the file-import lifecycle and shared
  runtime gate.
- Update `docs/CURRENT_STATE.md`, `docs/ROADMAP.md`, `plans/README.md`, and this
  ExecPlan at every checkpoint and on final M7 closure.
- Record the completed milestone in `~/.claude/memory.db` without secret values
  after repository documentation is current.

## Outcomes

Pending implementation and manual acceptance.
